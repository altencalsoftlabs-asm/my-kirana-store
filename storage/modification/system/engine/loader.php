...
//PATCH @4945 OVERRIDING			
//$base_u=str_replace('system/','',DIR_SYSTEM);
//$insidefolder='override/'.str_replace($base_u,'',DIR_APPLICATION);
//$file_override  = DIR_MODIFICATION . $insidefolder. 'model/' . $route . '.php';
            $file_override = DIR_APPLICATION_OVERRIDE . 'model/' . $route . '.php';
            $class_override = 'Kiranamodel' . preg_replace('/[^a-zA-Z0-9]/', '', $route);
//END PATCH @4945 OVERRIDING
            ...
            if (is_file($file_override)) {
                ...
                /*foreach (get_class_methods($class_override) as $method) {
                    $proxy->{$method} = $this->callback($this->registry, $route . '/' . $method);
                }*/

                $this->registry->set('model_' . str_replace('/', '_', (string)$route), new $class_override($this->registry));
    ...
    public function library($route)
    {
        // Sanitize the call
        $route = preg_replace('/[^a-zA-Z0-9_\/]/', '', (string)$route);
        //library override @4945 ASM 16-May-2018
        $file_override = DIR_MODIFICATION . 'system/library/' . $route . '.php';
        $class_override = str_replace('/', '\\', $route);
    ...
    public function helper($route)
    {
        ...
        $file_override = DIR_MODIFICATION . 'system/helper/' . preg_replace('/[^a-zA-Z0-9_\/]/', '', (string)$route) . '.php';

        //helper override @4945 ASM 16-May-2018
